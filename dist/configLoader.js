...
function loadEncryptedKeys() {
    ...
    try {
        ...
        if (key.length !== 32)
            throw new Error(`keys.key 不是 32 字节 hex(实际 ${key.length} 字节)`);
        const { iv, tag, data } = JSON.parse(fs.readFileSync(KEYS_FILE, 'utf-8'));
        if (!iv || !tag || !data)
            throw new Error('keys.enc 结构不完整(缺 iv/tag/data)');
 ...
 */
function applyApiKey(envName, fromKeys, fromCfg) {
    if (process.env[envName])
        return;
    const k = (fromKeys ?? '').trim();
    if (k) {
        process.env[envName] = k;
        return;
    }
    const c = (fromCfg ?? '').trim();
    if (c)
        process.env[envName] = c;
}
try {
