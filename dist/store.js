...
export async function saveMemory(params) {
  ...
  `).get(project, text?.trim() || '');
    ...
    const storeTx = db.transaction(() => {
        db.prepare(`
      INSERT INTO memory (id, text, project, session_id, type, mem_type, category, subcategory, tags, emotional_impact, importance, character_id, source, subject, tier, expires_at, is_active, created_at, updated_at, last_accessed_at, accessed_count, reference_count, locked, metadata, title, status, score_confidence, score_impact, score_priority, score_urgency, identity_locked)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(record.id, record.text, record.project, record.sessionId, record.type, record.memType, record.category, record.subcategory, JSON.stringify(record.tags), record.emotionalImpact, record.importance, record.characterId, record.source, record.subject, record.tier, record.expiresAt, 1, record.createdAt, record.updatedAt, record.lastAccessedAt, 0, 0, 0, record.metadata, record.title, record.status, record.scoreConfidence, record.scoreImpact, record.scorePriority, record.scoreUrgency, record.identityLocked ? 1 : 0);
