...
function memorySnapshotWarn(createdAt: string): string | null {
  ...
  const exactDate = new Date(createdAt).toISOString().slice(0, 10);
  return `> ⚠️ [Memory Snapshot Warning] 该记忆记录于 ${exactDate}(约 ${diffDays} 天前),属于历史快照,引用前请以最新对话/代码为准`;
}
...
async function main() {
  ...
  console.error('[castalia-anima] v2.0.0 started — emotional memory MCP server');
