...
export const CHAR_ID: string = process.env.CHAR_ID || 'airi';
 ...
 * charId 允许中文。
 */
export function charFor(project?: string): string {
 ...
 */
export function getPersona(project?: string): { charId: string; name: string; persona: string } {
 ...
 */
export const EMBED_MODE: string = (process.env.EMBED_MODE || 'ollama').toLowerCase();
...
/** MCP server 自描述名称 */
export const SERVER_NAME: string = process.env.MCP_SERVER_NAME || 'airi-memory';

/** 服务器版本号 */
export const SERVER_VERSION: string = process.env.MCP_SERVER_VERSION || '5.0.0';
