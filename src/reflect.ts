...
import { saveFacts, saveMemory, reEmbedMemory, batchEmbedPending, findDbByMemoryId } from './store.js';
...
export interface ReflectMemory {
  ...
  tags: string[];
  importance: number;
  emotionalImpact?: number;  // Anima: 情感冲击标量(0-10)，可选兼容
  subject: string;
  source: string;
  tier: string;
        ...
        case 'reclassify': {
          ...
          if (updates.length > 0) {
            ...
            if (_recR.changes > 0 && updates.includes('text = ?')) {
              try {
                const info = _tdb.prepare('SELECT rowid FROM memory WHERE id LIKE ? LIMIT 1').get(tid + '%') as any;
                if (info) _tdb.prepare('DELETE FROM vec_memory WHERE rowid = ?').run(BigInt(info.rowid));
              } catch {}
            }
            if (receipt.status === 'applied') result.applied++;
