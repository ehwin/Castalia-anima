        ...
        case 'reclassify': {
          ...
          if (updates.length > 0) {
            ...
            if (_recR.changes > 0 && updates.includes('text = ?')) {
              try {
                const info = db.prepare('SELECT rowid FROM memory WHERE id LIKE ? LIMIT 1').get(tid + '%') as any;
                if (info) db.prepare('DELETE FROM vec_memory WHERE rowid = ?').run(BigInt(info.rowid));
              } catch {}
            }
            if (receipt.status === 'applied') result.applied++;
